...
// For example if the polygon is really far away and only makes up 8 pixels on the screen then we use the sample state to figure out
// which pixels or what combination of pixels will actually be drawn from the original texture.
