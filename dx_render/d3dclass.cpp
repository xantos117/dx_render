	...
	float screenDepth, float screenNear)
{
	...
	// We query for the numerator and denominator values, then pass them to DirectX during the setup and it will calculate the proper refresh rate.
	...
	// [TODO] Perhaps look here for creating a frame timing? Or at least initializing it?
	...
	// then you can replace D3D_DRIVER_TYPE_HARDWARE with D3D_DRIVER_TYPE_REFERENCE and DirectX will use your CPU to draw instead of the video card hardware. 
	...
	// To get around this you will need to not use the default device and instead
	// enumerate all the video cards in the machine, have the user choose which one to use and then specify that card when creating the device.
	...
	// The reason for this is that once your polygons are sorted and rasterized, they just end up being colored pixels in this 2D buffer.
	...
	// By default DirectX already has a rasterizer state set up and working the exact same as the one below, but you have no control over it unless you set up one yourself.
	...
	// It will also be used to rotate, translate, and scale our objects in 3D space.
	// At the start we will just initialize the matrix to the identity matrix and keep a copy of it in this object.
	// The copy will be needed to be passed to the shaders for rendering as well.
...
//The Shutdown function will releaseand clean up all the pointers used in the Initialize function, its pretty straight forward.
...
// The first two are BeginSceneand EndScene.
